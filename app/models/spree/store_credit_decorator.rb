Spree::StoreCredit.class_eval do
  ...
  after_commit :send_credit_reward_information, on: :create, if: :referral_category?

  private
  def referral_category?
    category.try(:name) == REFERRAL_STORE_CREDIT_CATEGORY
  end

  def send_credit_reward_information
    Spree::ReferralMailer.credits_awarded(user, amount).deliver_now
  end
end
